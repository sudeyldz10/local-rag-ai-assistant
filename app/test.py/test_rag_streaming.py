"""Tests for rag_streaming.py: think-block streaming, the Turkish
follow-up bug, and min_score_threshold filtering."""
...
def test_repeating_text_is_flagged():
    assert s._is_repeating("z" * 60 * 4) is True


def test_own_regex_matches_turkish():
    assert bool(s.VAGUE_FOLLOWUP.search("bu ne demek")) is True


def test_bug_runtime_function_is_english_only():
    # rag_streaming.py defines its own Turkish-aware regex above, but
    # stream_ask_question actually calls this function instead:
    assert s._is_vague_followup is rag_pipeline._is_vague_followup
    assert s._is_vague_followup("bu ne demek") is False  # bug: not detected
...
if __name__ == "__main__":
    run({
        "repeating text is flagged": test_repeating_text_is_flagged,
        "streaming's own regex matches Turkish": test_own_regex_matches_turkish,
        "BUG: runtime function is English-only, misses Turkish": test_bug_runtime_function_is_english_only,
